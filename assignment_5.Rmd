...
Salamander counts were highest for both sections of the forest between 2003-2010. The old growth section of the forest typical had higher counts of salamanders than the clear cut sections except for the years 2015-2017.
...
```
There is not a significant association between forest condition (old growth or clear cut) and where salamanders are found in the creek channel. Counts and proportions (see Table 1.) differ between channel type and forest section significantly (X^2^(2) = `r round(salamander_2017_chi$statistic, 2)`, p-value = `r round(salamander_2017_chi$p.value, 2)`). Majority of salamanders are found in the "cascade" channel type regardless of whether it's located in a clear cut or old growth section of the forest. 
...
```
The difference in mean salamander weights between clear cut and old growth sections has a neglible effect size (cohen's d = `r round(salamander_section_d$estimate, 3)`). There is not a statiscal signficance (t(`r round(salamander_section_ttest$parameter, 2)` = `r round(salamander_section_ttest$statistic, 2)`, p = `r round(salamander_section_ttest$p.value, 2)`)) between the mean weights of salamanders located in clear cut and old growth forest sections.
